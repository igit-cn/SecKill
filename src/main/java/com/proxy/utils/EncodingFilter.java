...
public class EncodingFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws ServletException, IOException {
        if (request.getMethod().equals("POST")) {//POST编码额外处理
            chain.doFilter(new GetHttpServletRequestWrapper2(request), response);
            return;
        }
        // 设置请求响应字符编码  
    ...
    class GetHttpServletRequestWrapper2 extends HttpServletRequestWrapper {

        public GetHttpServletRequestWrapper2(HttpServletRequest request) {
            super(request);
        }
    }
}  